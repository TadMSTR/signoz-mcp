"""Pre/post extension-hook registry for signoz-mcp.
...
without editing the server. Because this is a single-server process, hooks key on the
tool name alone.

API::

    from signoz_mcp.hooks import register_before, register_after

    async def scope_time(kwargs: dict) -> dict:
        kwargs.setdefault("start", "-15m")
        return kwargs

    register_before("search_traces", scope_time)

    async def tag(result):
        return {"wrapped": result}

    register_after("query_metric", tag)
...
def register_before(tool: str, handler: Callable[..., Any]) -> None:
    ...
    Args:
        tool: the tool's name (its Python function name, e.g. ``"search_traces"``).
...
def register_after(tool: str, handler: Callable[..., Any]) -> None:
    ...
    Args:
        tool: the tool's name (its Python function name, e.g. ``"query_metric"``).
